...
namespace Core.Web.Models.JQueryDataTables
{
    public class JQueryDataTablesModelBinder : IModelBinder
    {
        ...
        {
            _bindingContext = bindingContext ?? throw new ArgumentNullException("bindingContext");
            ...
            if (dataTablesRequest.iSortingCols.HasValue)
            {
                    ...
                    != dataTablesRequest.iSortCol_.Count)
                {
                    throw new InvalidOperationException(
                        $"Amount of items contained in iSortCol_ {dataTablesRequest.iSortCol_.Count} do not match the amount specified in iSortingCols which is {dataTablesRequest.iSortingCols.Value}");
                }
                ...
                {
                    throw new InvalidOperationException(
                        $"Amount of items contained in sSortDir_ {dataTablesRequest.sSortDir_.Count} do not match the amount specified in iSortingCols which is {dataTablesRequest.iSortingCols.Value}");
                }
            }
        ...
        private ReadOnlyCollection<string> GetStringList(string key)
        {
            ...
            while (hasMore)
            {
                ...
                list.Add((string)Convert.ChangeType(valueResult.FirstValue, typeof(string)));
        ...
        private ReadOnlyCollection<T> GetAList<T>(string key) where T : struct
        {
            ...
            while (hasMore && i <= 5)
            {
                ...
                string newKey1 = newKey.ToString();
                ...
                list.Add((T)Convert.ChangeType(valueResult.FirstValue, typeof(T)));
                i++;
            }
        ...
        private string GetString(string key)
        {
            ...
            return (string)Convert.ChangeType(valueResult.FirstValue, typeof(string));
        }
        ...
        private T GetA<T>(string key) where T : struct
        {
            ...
            return (T)Convert.ChangeType(valueResult.FirstValue, typeof(T));// (T)valueResult.ConvertTo(typeof(T));  (T)valueResult.ConvertTo(typeof(T)); (T)Convert.ChangeType(valueResult, typeof(string));
        }
        ...
        private T? GetANullableValue<T>(string key) where T : struct
        {
            ...
            return (T?)Convert.ChangeType(valueResult.FirstValue, typeof(string)); //TypeDescriptor.GetConverter(typeof(T))
        }
        ...
        public Task BindModelAsync(ModelBindingContext bindingContext)
        {
            ...
            _bindingContext.Result = ModelBindingResult.Success(dataTablesRequest);
